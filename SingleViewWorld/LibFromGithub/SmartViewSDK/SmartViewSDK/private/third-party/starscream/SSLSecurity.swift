...
public class SSLSecurity {
    ...
    */
    public convenience init(usePublicKeys: Bool = false) {
      //  let paths = NSBundle.mainBundle().pathsForResourcesOfType("der", inDirectory: ".")
    ...
    */
    public func isValid(_ trust: SecTrust, domain: String?) -> Bool {
        ...
        if self.usePublicKeys {
            ...
            var result: SecTrustResultType = SecTrustResultType(rawValue: SecTrustResultType.unspecified.rawValue)!
            
            SecTrustEvaluate(trust,&result)
            //if result == .Unspecified || result == .Proceed {
            if result == SecTrustResultType(rawValue: SecTrustResultType.unspecified.rawValue)! || result == SecTrustResultType(rawValue: SecTrustResultType.proceed.rawValue)! {
                var trustedCount = 0
