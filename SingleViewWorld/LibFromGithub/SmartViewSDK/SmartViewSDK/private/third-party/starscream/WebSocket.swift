    ...
    */
    ...
    private func initStreamsWithData(_ data: Data, _ port: Int) {
        ...
        writeQueue.addOperation { [weak self] in
            ...
            if(self!.certValidated != false){
                self?.isSent = true
                outStream.write(bytes, maxLength: data.count)}
            else
            {
                self?.tempData = data as NSData
                self?.isSent = false
            }
        }
    }
        ...
        if let sec = security, !certValidated && [.hasBytesAvailable, .hasSpaceAvailable].contains(eventCode) {
            ...
            if sec.isValid(trust as! SecTrust, domain: domain) {
                certValidated = true
                    if(self.isSent != true)
                    {
                        let bytes = self.tempData.bytes.assumingMemoryBound(to: UInt8.self)
                        let outStream = self.outputStream
                        outStream!.write(bytes, maxLength: self.tempData.length)
                        self.isSent = true
                    }
